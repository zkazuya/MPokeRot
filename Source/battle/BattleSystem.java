...
public class BattleSystem {
    ...
    public void update() {
        ...
        else { // UTILIZE OOP AND MADE THE WHOLE LOGIC USE ENHANCED SWITCH
    ...
    public void initiateEnemyTakingDamageAnimationState() { // THIS STATE IS JUST RENDERING THE HP BAR OF ENEMY POKEROT DRAINING
        if (activeEnemy.getDrawnHP() == activeEnemy.getCurrentHP()) {
            if (activeEnemy.getCurrentHP() <= 0) { // IF THE ENEMY DIED STOP AND SHOW FAINTED DIALOGUE
    ...
    public void initiateOurPokerotTakingDamageAnimationState() { // THIS STATE RENDERS OUR HP BAR MOVING (DAMAGE & HEAL)
        ...
        if (activePlayer.getDrawnHP() == activePlayer.getCurrentHP()) { 
            if (activePlayer.getCurrentHP() <= 0) { // IF THE ACTIVE POKEROT FAINTED (HP <= 0)
                if (!dialogText.contains("fainted")) {
    ...
    public void initiateVictoryState () { // THIS STATE IS POKEROT GAINING EXP AND DETECTING IF LVL UP
        if (gamePanel.keyHandler.getEnterPressed()) {
            ...
            } else checkNextEnemy(); // HELPER METHOD TO SEE IF BATTLE CONTINUES OR ENDS 
            keyCooldown = 16;
        }
    }
    ...
    public void initiateSwapSelectionState () {
        ...
        if (gamePanel.keyHandler.getEnterPressed()) {
            ...
            } else dialogText = "There's no PokeRot in that slot!"; // THEY PICKED AN EMPTY SLOT
            keyCooldown = 16;
        }
    }
    ...
    private void checkNextEnemy () {
        ...
        if (nextEnemy != null) {
            ...
            gamePanel.battleUI.loadFighterImages(activePlayer, activeEnemy);
            if (currentNPCOpponent != null) dialogText = currentNPCOpponent.getName() + " sent out " + activeEnemy.getName() + "!";
            else dialogText = "A wild " + activeEnemy.getName() + " appeared!";
            battleSubState = 9; // ROUTE TO TRANSITION SCREEN
        } else {
            if (currentNPCOpponent != null) dialogText = "You defeated " + currentNPCOpponent.getName() + "!";
            else dialogText = "You won the battle!";
            battleSubState = 9;
        }
    }
