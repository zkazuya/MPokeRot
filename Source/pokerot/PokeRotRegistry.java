...
public class PokeRotRegistry {
    ...
    public void setPokeRotMovesOnEach () { // THIS METHOD GIVES THE ENEMY AI MOVES TO USE
        for (PokeRot eachPokeRot : pokeRotList) {
            // Create a temporary list to keep track of the moves we are handing out
            ArrayList<Move> assignedMoves = new ArrayList<>();
            
            // Keep looping until this PokeRot has exactly 4 unique moves
            while (assignedMoves.size() < 4) { 
                int randomMove = (int) (Math.random() * pokeRotMoves.size()); 
                Move newMove = pokeRotMoves.get(randomMove);
                
                // Check if we ALREADY gave them this exact move
                if (!assignedMoves.contains(newMove)) {
                    // If it's a new move, add it to our tracker AND give it to the PokeRot
                    assignedMoves.add(newMove);
                    eachPokeRot.addMove(newMove); 
                }
            }
        }
    }
