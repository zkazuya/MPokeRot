...
public class TileManager {
    ...
    public TileManager(GamePanel gamePanel) {
        ...
        mapTileNumber = new int[gamePanel.getMaxScreenColumn()][gamePanel.getMaxScreenRow()]; // the tile number's max
                                                                                              // size is just the whole
                                                                                              // map
        getTileImage(); // puts every tiles into tile array when tile manager is instantiated
        loadMap("Assets/Maps/Map-save-updated-final.txt"); // tile manager is created once so this loads once! separate
                                                           // methods can call this method
    }

    public void getTileImage() {
        try {
            // wrapped this in try catch so the program won't crash also to show the error
            tile[0] = new Tile();
